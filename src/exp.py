...
def run_experiment(dataset, optimizer, optimizer_name, batch_size=100, num_epochs=10, data_dir='/home/app/datasets',
    ...
    training_losses, test_losses, training_accuracies, test_accuracies, model = utils.train(train_loader, test_loader,
                                                                                     use_gpu=use_gpu,
                                                                                     create_graph=create_graph,
                                                                                     num_epochs=num_epochs,
                                                                                     optimizer=optimizer, **kwargs)
    return {'dataset': dataset, 'optimizer': optimizer_name, 'training_losses': training_losses,
            'test_losses': test_losses, 'training_accuracies': training_accuracies,
            'test_accuracies': test_accuracies}, model
...
def run_experiment_all_optimizers(dataset, optimizers, num_epochs=100, save_dir='/home/app/results'):
    ...
    for optimizer in optimizers:
        results.append(run_experiment(dataset, optimizer, optimizer_args[optimizer]['name'], batch_size=batch_size,
                                      num_epochs=num_epochs,
